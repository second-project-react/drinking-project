...
const MultiIngredient = () => {
   ...
   useEffect(() => {
      Axios.get(
         `https://www.thecocktaildb.com/api/json/v2/${process.env.REACT_APP_API_KEY}/list.php?i=list`
      )
         .then((response) => {
   ...
   useEffect(() => {
      ...
      Axios.get(
         `https://www.thecocktaildb.com/api/json/v2/${process.env.REACT_APP_API_KEY}/filter.php?i=${ingString}`
      )
         .then((response) => {
   ...
   const listItems = ingredients
      .filter((item) =>
         item.strIngredient1.toLowerCase().includes(searchValue.toLowerCase())
      )
      .map((item, index) => {
         return (
            ...
            <div
               className={item.selected ? 'chipMain chipActive' : 'chipMain'}
   ...
   return (
      <div className="mainDiv">
         <h2 className="drinkType">Select By Ingredients</h2>
         <h5 className="qoute">
            "Here’s to the nights we don’t remember and the friends we won’t
            forget."
         </h5>
               ...
               <Row>
                  <Col s={12} xs={12} md={10} l={4} xl={4}>
                     <div className="selectBox">
                        ...
                        <div>
                           <p className="multiSelected">
                              {' '}
                              Selected Ingredients :{' '}
                           </p>

                           {selectIngredient.map((items, index) => (
                              <button
                                 class="chip"
                                 onClick={() =>
                                    deleteIngredient(
                     ...
                     <div className="multiCardList">
                        {isLoading ? (
                           ...
                           displayItems.map((items) => (
                              <IngredientCard cocktailList={items} />
                           ))
                        ) : (
